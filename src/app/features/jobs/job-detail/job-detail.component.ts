  ...
  template: `
    <div class="bg-slate-50 min-h-screen pb-20">
      <div class="max-w-5xl mx-auto px-6 pt-12">
        <!-- Back Button -->
        <button (click)="goBack()" class="flex items-center text-slate-500 hover:text-orange-600 mb-10 transition-colors group font-bold text-sm tracking-tight">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div class="space-y-4">
                 <div class="flex items-center gap-3">
                   <div class="w-12 h-12 bg-orange-50 border border-orange-100 rounded-xl flex items-center justify-center">
                      <span class="text-orange-600 font-black text-xl">{{ job.company.substring(0, 1) }}</span>
                   </div>
              ...
              <div class="flex flex-col gap-3 min-w-[240px]">
                 <a [href]="job.url" target="_blank" (click)="onApply($event)"
                   class="px-8 py-4 bg-slate-900 hover:bg-orange-600 text-white font-black rounded-2xl shadow-xl shadow-slate-200 transition-all transform hover:-translate-y-1 text-center">
                   Apply for this position
                 </a>
                 <button 
                  (click)="onFavorite()"
                  class="px-8 py-4 bg-white border-2 font-bold rounded-2xl transition-all flex items-center justify-center gap-2"
                  [ngClass]="(isBookmarked$ | async) ? 'border-orange-100 bg-orange-50 text-orange-500' : 'border-slate-100 text-slate-600 hover:border-orange-200 hover:text-orange-500 hover:bg-orange-50/30'"
                 >
                    <svg class="h-5 w-5" [attr.fill]="(isBookmarked$ | async) ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                    </svg>
                    {{ (isBookmarked$ | async) ? 'Saved to Favorites' : 'Save for later' }}
                 </button>
              </div>
            </div>
          </div>
                ...
                <div class="space-y-6">
                  ...
                  <div class="flex items-start gap-4">
                     ...
                     <div>
                        ...
                        <p class="text-orange-700 font-black text-xl mt-0.5 tracking-tighter">{{ job.salary || 'Competitive' }}</p>
                     </div>
                  </div>
                </div>
              </div>
              
              <div class="bg-orange-600 rounded-3xl p-8 text-white shadow-xl shadow-orange-100 overflow-hidden relative group">
                <div class="relative z-10">
                  <h3 class="text-xl font-bold mb-2">Want updates?</h3>
                  <p class="text-orange-100 text-sm font-medium mb-6 leading-relaxed">Join our mailing list to get the latest jobs in this category sent straight to your inbox.</p>
                  <button class="w-full py-3 bg-white text-orange-600 font-black rounded-xl hover:bg-orange-50 transition-all">Enable Alerts</button>
                </div>
        ...
        <div *ngIf="!job" class="flex flex-col items-center justify-center py-40 animate-fade-in">
          <div class="inline-block animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-orange-600"></div>
