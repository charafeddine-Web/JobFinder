...
@Component({
  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-[#F8FAFC] p-6 relative overflow-hidden selection:bg-indigo-500 selection:text-white">
      <!-- Background Decorative Elements -->
      <div class="absolute top-[-10%] right-[-5%] w-[50%] h-[50%] bg-indigo-100/40 rounded-full blur-[120px] animate-float"></div>
      <div class="absolute bottom-[-10%] left-[-5%] w-[40%] h-[40%] bg-emerald-100/40 rounded-full blur-[100px] animate-float" style="animation-delay: 2s"></div>
          ...
          <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="space-y-6">
            ...
            <div class="text-center pt-2">
              <p class="text-slate-500 font-medium">
                New to JobFinder? 
                <a routerLink="/auth/register" class="font-bold text-indigo-600 hover:text-indigo-700 hover:underline decoration-2 underline-offset-4 transition-all">Create an account</a>
              </p>
            </div>
          </form>
        </div>
